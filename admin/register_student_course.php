...
// Alternative approach if you want to be more flexible with general courses:
// This version allows any student to take general courses as long as they meet minimum requirements

/*
// ALTERNATIVE: More flexible general course logic
if ($eligibility['is_general'] == 1) {
    $course_level = $eligibility['level'] ?? 1;
    $min_level = $eligibility['min_level'] ?? $course_level;
    $max_level = $eligibility['max_level'] ?? null; // No upper limit by default
    
    // Check minimum level requirement
    if ($student['student_level'] >= $min_level) {
        // If max_level is set, check it; otherwise allow any level above minimum
        if ($max_level === null || $student['student_level'] <= $max_level) {
            $eligible = true;
            
            if ($student['student_level'] == $course_level) {
                $eligibility_message = "General course - student is at the target level ({$course_level}).";
            } else {
                $eligibility_message = "General course - accessible to all students (Level {$student['student_level']} taking Level {$course_level} course).";
            }
        } else {
            $eligible = false;
            $eligibility_message = "Student level ({$student['student_level']}) exceeds the maximum allowed level ({$max_level}) for this course.";
        }
    } else {
        $eligible = false;
        $eligibility_message = "Student level ({$student['student_level']}) is below the minimum required level ({$min_level}) for this course.";
    }
}
*/

// ADDITIONAL CONSIDERATIONS FOR GENERAL COURSES:

// 1. Prerequisites check (if you have a prerequisites table)
/*
if ($eligible && $eligibility['is_general'] == 1) {
    // Check if student has completed prerequisites
    $prereq_stmt = $conn->prepare("
        SELECT cp.prerequisite_course_id, c.course_code, c.course_title
        FROM course_prerequisites cp
        JOIN courses c ON cp.prerequisite_course_id = c.id
        WHERE cp.course_id = :course_id
    ");
    $prereq_stmt->execute(['course_id' => $course_id]);
    ...
    if (!empty($prerequisites)) {
        foreach ($prerequisites as $prereq) {
            ...
            $completed_stmt = $conn->prepare("
                ...
                JOIN course_results cr ON sc.id = cr.student_course_id
                WHERE sc.student_id = :student_id 
                AND sc.course_id = :prereq_course_id 
                AND cr.grade IN ('A', 'B', 'C', 'D') -- Passing grades
            ");
            ...
            if ($completed_stmt->fetch()['count'] == 0) {
                $eligible = false;
                $eligibility_message = "Student has not completed prerequisite course: {$prereq['course_code']} - {$prereq['course_title']}.";
                break;
            }
        }
    }
...
// 2. Credit load check (to prevent overloading)
/*
if ($eligible) {
    // Check current semester credit load
    $credit_stmt = $conn->prepare("
        SELECT SUM(c.credit_units) as total_credits
    ...
    $current_credits = $credit_stmt->fetch()['total_credits'] ?? 0;
    $new_total = $current_credits + $course['credit_units'];
    $max_credits = 24; // Maximum credits per semester
    
    if ($new_total > $max_credits) {
        $eligible = false;
        $eligibility_message = "Adding this course would exceed maximum credit limit ({$max_credits}). Current: {$current_credits}, Course: {$course['credit_units']}, Total would be: {$new_total}.";
    }
}
*/
